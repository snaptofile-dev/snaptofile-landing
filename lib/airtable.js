...
/**
 * Find a license record by license key. Returns the Airtable record
 * (with .id and .fields) or null if not found.
 *
 * The lookup is normalized to uppercase before querying, which matches the
 * canonical key format we always write (Crockford base32, uppercase). User
 * input is normalized at the API endpoint layer via normalizeLicenseKey().
 *
 * Used by /api/activate and /api/validate (Chunk 5).
 */
export async function findByLicenseKey(licenseKey) {
  if (!licenseKey) return null;
  const normalized = String(licenseKey).trim().toUpperCase();
  const escaped = normalized.replace(/'/g, "\\'");
  const formula = encodeURIComponent(`{license_key} = '${escaped}'`);
  const url = `${baseUrl()}?filterByFormula=${formula}&maxRecords=1`;

  const res = await fetch(url, { method: 'GET', headers: authHeaders() });
  if (!res.ok) {
    const body = await res.text();
    throw new Error(`Airtable findByLicenseKey failed (${res.status}): ${body}`);
  }
  const data = await res.json();
  return (data.records && data.records[0]) || null;
}
